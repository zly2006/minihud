...
public class OverlayRendererLightLevel extends OverlayRendererBase {
    ...
    private static final Identifier TEXTURE_NUMBERS = new Identifier(Reference.MOD_ID,
            "textures/misc/light_level_numbers.png");
    ...
    public void update(Vec3d cameraPos, Entity entity, MinecraftClient mc) {
        ...
        // (double) (System.nanoTime() - pre) / 1000000000D);
    ...
    public void allocateGlResources() {
        ...
        this.allocateBuffer(VertexFormat.DrawMode.DEBUG_LINES, VertexFormats.POSITION_COLOR,
    ...
    private void renderLightLevels(Vec3d cameraPos, MinecraftClient mc) {
        ...
        if (count > 0) {
            ...
            Direction numberFacing = Configs.Generic.LIGHT_LEVEL_NUMBER_ROTATION.getBooleanValue()
                    ? mc.player.getHorizontalFacing()
                    : Direction.NORTH;
            LightLevelNumberMode numberMode = (LightLevelNumberMode) Configs.Generic.LIGHT_LEVEL_NUMBER_MODE
                    .getOptionListValue();
            LightLevelMarkerMode markerMode = (LightLevelMarkerMode) Configs.Generic.LIGHT_LEVEL_MARKER_MODE
                    .getOptionListValue();
    ...
    private void renderMarkers(IMarkerRenderer renderer,
        ...
        LightLevelRenderCondition condition = (LightLevelRenderCondition) Configs.Generic.LIGHT_LEVEL_MARKER_CONDITION
                .getOptionListValue();
    ...
    private void renderLightLevelNumbers(double dx, double dy, double dz,
            ...
            Color4f colorDark,
            BufferBuilder buffer) {
        LightLevelRenderCondition condition = (LightLevelRenderCondition) Configs.Generic.LIGHT_LEVEL_NUMBER_CONDITION
                .getOptionListValue();
        boolean autoHeight = Configs.Generic.LIGHT_LEVEL_AUTO_HEIGHT.getBooleanValue();
        Color4f color;
    ...
    private void renderLightLevelTextureColor(double x, double y, double z, Direction facing, int lightLevel,
    ...
    private void renderLightLevelCross(double x, double y, double z, Color4f color, double offset1, double offset2,
            BufferBuilder buffer) {
        y += 0.005;
    ...
    private void renderLightLevelSquare(double x, double y, double z, Color4f color, double offset1, double offset2,
            BufferBuilder buffer) {
        y += 0.005;
    ...
    private void updateLightLevels(World world, BlockPos center) {
        ...
        for (int cx = minCX; cx <= maxCX; ++cx) {
            ...
            for (int cz = minCZ; cz <= maxCZ; ++cz) {
                ...
                int yMaxMath = chunk.getHighestNonEmptySection(); // Replaces getHighestNonEmptySectionYOffset()
                yMaxMath = yMaxMath == -1 ? chunk.getBottomY()
                        : ChunkSectionPos.getBlockCoord(chunk.sectionIndexToCoord(yMaxMath));
                    ...
                    for (int x = startX; x <= endX; ++x) {
                        for (int z = startZ; z <= endZ; ++z) {
                            ...
                            if ((collisionCheck == false || state.getCollisionShape(chunk, mutablePos).isEmpty()) &&
                                ...
                                int block = y < worldTopHeight
                                        ? lightingProvider.get(LightType.BLOCK).getLightLevel(mutablePos)
                                        : 0;
                                int sky = y < worldTopHeight
                                        ? lightingProvider.get(LightType.SKY).getLightLevel(mutablePos)
                                        : 15;
                                double topY = state.getOutlineShape(chunk, mutablePos).getMax(Direction.Axis.Y);

                                // Don't render the light level marker if it would be raised all the way to the
                                // next block space
                                if (autoHeight == false || topY < 1) {
                                    ...
                                    // y += 2; // if the spot is spawnable, that means the next spawnable spot can
                                    // be the third block up
                                }
                            }
                        }
                    }
    ...
    private boolean canSpawnAtWrapper(int x, int y, int z, Chunk chunk, World world, boolean skipBlockCheck) {
        try {
            ...
            InfoUtils.showGuiOrInGameMessage(Message.MessageType.WARNING, 8000,
                    "This dimension seems to have missing block tag data, the light level will not use the normal block spawnability checks in this dimension. This is known to happen on some Waterfall/BungeeCord/ViaVersion/whatever setups that have an older MC version at the back end.");
            tagsBroken = true;
    ...
    /**
     * This method mimics the one from WorldEntitySpawner, but takes in the Chunk to
     * avoid that lookup
     */
    private boolean canSpawnAt(int x, int y, int z, Chunk chunk, World world, boolean skipBlockCheck) {
        ...
        if ((skipBlockCheck && stateDown.isAir() == false && (stateDown.getBlock() instanceof FluidBlock) == false) ||
            ...
            if (isClearForSpawnWrapper(world, this.mutablePos, state, state.getFluidState(),
                    EntityType.WITHER_SKELETON)) {
                this.mutablePos.set(x, y + 1, z);
                BlockState stateUp1 = chunk.getBlockState(this.mutablePos);

                return isClearForSpawnWrapper(world, this.mutablePos, stateUp1, state.getFluidState(),
                        EntityType.WITHER_SKELETON);
            }
            ...
            if (state.getFluidState().isIn(FluidTags.WATER)) {
                ...
                return stateUp1.getFluidState().isIn(FluidTags.WATER) &&
                        chunk.getBlockState(this.mutablePos.set(x, y + 2, z)).isSolidBlock(world,
                                this.mutablePos) == false;
            }
        }
    ...
    public static boolean isClearForSpawnWrapper(BlockView blockView, BlockPos pos, BlockState state,
            FluidState fluidState, EntityType<?> entityType) {
        return tagsBroken ? isClearForSpawnStripped(blockView, pos, state, fluidState, entityType)
     ...
     * it removes any calls to BlockState.isIn(), which causes an exception on
     * certain
     * ViaVersion servers that have old 1.12.2 worlds.
     * (or possibly newer versions as well, but older than 1.16 or 1.15 or whenever
     * the tag syncing was added)
     */
    public static boolean isClearForSpawnStripped(BlockView blockView, BlockPos pos, BlockState state,
            FluidState fluidState, EntityType<?> entityType) {
